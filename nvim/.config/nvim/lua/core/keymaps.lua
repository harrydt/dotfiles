...
utils.map('n', '<leader>gdo', '<cmd>DiffviewToggle<CR>', opts)
utils.map('n', '<leader>gdc', '<cmd>DiffviewClose<CR>', opts)
utils.map('n', '<leader>gdf', '<cmd>DiffviewFileHistory<CR>', opts)
utils.map('n', '<leader>gdh', '<cmd>DiffviewFileHistory %<CR>', opts)
-- utils.map uses nvim.set_keymap which doesn't support lua functions directly
vim.keymap.set('n', '<leader>gdm', function() vim.cmd('DiffviewOpen ' .. utils.get_default_branch_name() .. '...HEAD') end, opts)
